...
public class CarController : MonoBehaviour
{
    ...
    public bool bl_CarReady;

	public bool bl_BoostAvailable;
	private BoostManager BM_Script;
	private bool bl_ResetAccerleration;
	public float fl_Speed = 0.75F;

	public bool bl_BrakeAvailable;
	private BrakeManager BRM_Script;
	private bool bl_ResetBrake;
	...
	void Awake ()
    {
		...
		if(bl_BoostAvailable)
		{
			BM_Script = GameObject.Find("BoostIcon").GetComponent<BoostManager>();
		}
		if(bl_BrakeAvailable)
		{
			BRM_Script = GameObject.Find("BrakeIcon").GetComponent<BrakeManager>();
		}
	...
	void FixedUpdate()
	{
        if (Application.loadedLevelName == "CoinPlacement")
        {
            if (bl_CarReady)
            {
                Move(NIR_Script.in_Horizontal, fl_Speed);
            }
        }
		if(Application.loadedLevelName != "CoinPlacement")
        {
	...
	public void Move (float steerInput, float accelBrakeInput)
    {
		if(bl_BoostAvailable)
		{
			if( BM_Script.bl_PlayerBoosting)
			{
				fl_Speed = 5F;
			}
